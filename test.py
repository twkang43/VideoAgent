...
def ask_gpt_caption_step_force(question, caption, num_frames):
    answer_format = {"final_answer": "xxx"}
    prompt = f"""
    Given a video that has {num_frames} frames, the frames are decoded at 1 fps. Given the following descriptions of the sampled frames in the video:
    {caption}
...
def select_best_sub_segment(question, captions, num_frames, segment_des):
    ...
    Our goal is to identify additional frames that contain crucial information necessary for answering the question. These frames should not only address the query directly but should also complement the insights gleaned from the descriptions of the initial frames.
    To achieve this, we will:
    1. Divide the video into two segments based on the intervals between the initial frames as, candiate segments: {segment_des}
    2. Determine which segment is likely to contain frames that are most relevant to the question. These frames should capture key visual elements, such as objects, humans, interactions, actions, and scenes, that are supportive to answer the question.
    For the frame identified as potentially relevant, provide a concise description focusing on essential visual elements. Use a single sentence per frame. If the specifics of a segment's visual content are uncertain based on the current information, use placeholders for specific actions or objects, but ensure the description still conveys the segment's relevance to the query.
    Return the descriptions and the segment id of the best segment in JSON format, note "segment_id" must be smaller than 3, "duration" should be the same as candiate segments:
    ```
    {formatted_description}
    ```
    """
...
def retrieve_best_frames(caps, duration, formatted_question):
    i, j = duration
    logger.info(f"duration : [{i}, {j}]")
    num_frames = j - i + 1
    
    if num_frames < SEGMENT_LENGTH_LIMIT+1:
    ...
    segment_des = {
        1: f"{caption_i}-{caption_k}", 2: f"{caption_k}-{caption_j}"
    }
    ...
    if parsed_candidate_descriptions["best_segment"]["segment_id"] == "1":
        return retrieve_best_frames(caps, [i, k], formatted_question)
    else:
        return retrieve_best_frames(caps, [k, j], formatted_question)
######################################################################################################
...
def run_one_question(video_id, ann, caps, logs):
    ...
    if confidence < 3:
        try:
            ...
            frame_idx = retrieve_best_frames(
            ...
            sampled_caps = read_caption(caps, sample_idx)
            previous_prompt, answer_str = ask_gpt_caption_step_force(
                formatted_question, sampled_caps, num_frames
            )
            answer = parse_text_find_number(answer_str)
            confidence_str = self_eval(previous_prompt, answer_str)
...
def main():
    ...
    tasks = [
        ...
        for video_id in list(anns.keys())[:TEST_SAMPLES]
    ]
